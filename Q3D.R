...
e = arpack(func, options=list(n=vcount(g), nev=3,ncv=8, which="LM", maxiter=200));
max_evec = abs(e$vectors[,1])
max_nodes = cbind(max_evec,seq(1,vcount(g)))
max_nodes = max_nodes[order(max_nodes[,1],decreasing=TRUE),]

remove_nodes = max_nodes[1:k,2]
...
plot(seq(4000,5500,100),svar,type="l",xlab="K",ylab="Effective Strength(beta=0.2,delta=0.7)")
