...
for(k in seq(200,600,100))
{
g=read.graph("C:\\Users\\Sagar\\Dropbox\\Fall13\\CSC591\\P4\\static.network",format=c("edgelist"),directed=FALSE)

gdeg=degree(g,v=V(g),loops=FALSE)
gdegree=cbind(gdeg,seq(1,vcount(g)))
gdegree=gdegree[order(gdegree[,1],decreasing=TRUE),]

remove_nodes=gdegree[1:k,]

g=delete.vertices(g,remove_nodes[,2])
m=get.adjacency(g)
...
plot(seq(200,600,100),svar,type="l",xlab="K",ylab="Effective Strength(beta=0.2,delta=0.7)")
...
gdegree=degree(g,v=V(g),loops=FALSE)
gdegree=cbind(gdegree,seq(1,vcount(g)))
gdegree=gdegree[order(gdegree[,1],decreasing=TRUE),]
